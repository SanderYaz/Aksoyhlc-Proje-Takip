...
//Projeye kullanıcı atanıp atanmadığını kontrol etme ve son eklenen projenin id sini alma
        	if (isset($_POST['proje_kullanici'])) {
        		$son_eklenen_id=$db->lastInsertId();


//Projeye kaç kişi atandıysa onlara mail gönderme ve kullanıcı-proje bağını veritabanına kayıt etme işlemi
        		foreach ($_POST['proje_kullanici'] as $deger) {
        			$proje_kul_bag_ekle=$db->prepare("INSERT INTO proje_kul_bag SET
        				proje_id=:proje_id,
        				kul_id=:kul_id,
        				kul_proje_yetki=:kul_proje_yetki
        				");

        			$ekle=$proje_kul_bag_ekle->execute(array(
        				'proje_id' => $son_eklenen_id,
        				'kul_id' => $deger,
        				'kul_proje_yetki'=>1
        			));
        ...
        if (isset($_POST['projeguncelle'])) {
            ...
            $isim=turkce_temizle(tum_bosluk_sil($isim1));
            ...
            $yukleme=$dosyayukleme->execute(array(
              ...
              'proje_id' => guvenlik($_POST['proje_id'])
          ));


            if (isset($_POST['proje_kullanici'])) {
              $proje_kul_bag_sil=$db->prepare("DELETE from proje_kul_bag where proje_id=:id");
              $silme=$proje_kul_bag_sil->execute(array(
  ...
  if (isset($_POST['siparisekle'])) {
   ...
   if (isset($_POST['sip_kullanici'])) {
      $son_eklenen_id=$db->lastInsertId();

      foreach ($_POST['sip_kullanici'] as $deger) {
         $siparis_kul_bag_ekle=$db->prepare("INSERT INTO sip_kul_bag SET
            sip_id=:sip_id,
            kul_id=:kul_id
            ");

         $ekle=$siparis_kul_bag_ekle->execute(array(
            'sip_id' => guvenlik($son_eklenen_id),
            'kul_id' => guvenlik($deger)
        ));
     }
 }
...
if (isset($_POST['siparisguncelle'])) {
   $siparisguncelle=$db->prepare("UPDATE proje SET
      musteri_isim=:isim,
      musteri_mail=:mail,
      musteri_telefon=:telefon,
      sip_baslik=:baslik,
  ...
  if (isset($_POST['sip_kullanici'])) {
      $sip_kul_bag_sil=$db->prepare("DELETE from sip_kul_bag where sip_id=:id");
      $silme=$sip_kul_bag_sil->execute(array(
         'id' => guvenlik($_POST['sip_id'])
     ));

      foreach ($_POST['sip_kullanici'] as $deger) {
         $sip_kul_bag_ekle=$db->prepare("INSERT INTO sip_kul_bag SET
            sip_id=:sip_id,
            kul_id=:kul_id
            ");

         $ekle=$sip_kul_bag_ekle->execute(array(
            'sip_id' => guvenlik($_POST['sip_id']),
            'kul_id' => guvenlik($deger)
        ));
     }
 }
